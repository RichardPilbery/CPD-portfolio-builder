...
<header class="flex flex-wrap items-center mb-3 py-2">
        <p class="w-full text-gray-500"><a href="/home">Home</a> / <a href="/portfolio">My Portfolio Entries</a> / Create Portfolio Entry</p>
        <div class="flex justify-between items-center w-full">
            <h2><i class="fas fa-book"></i> Create Portfolio Entry</h2>
        </div>
</header>
    ...
    <div class="w-full sm:w-3/4 md:w-3/4 lg:w-3/4 px-2">
        <form method="POST" action="/portfolio" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <div class="flex">
                    <div class="w-3/4">
                        ...
                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->has('actdate') ? 'is-invalid' : '' }}" id="actdate"  name="actdate" type="date" value="{{ old('actdate', date('Y-m-d')) }}">
                    ...
                    <div class="w-1/4 pl-4">
                        ...
                        <input class="float-right" type="checkbox" name="profile" id="profile">
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->has('title') ? 'is-invalid' : '' }}" id="title" name="title" type="text" placeholder="Title" value="{{ old('title') }}">
                @if($errors->has('title'))
                    <div class="error-fb">
            ...
            <div class="mb-4">
                ...
                <textarea rows="4" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="description" name="description" placeholder="Provide a brief description of your portfolio entry">{{old('description')}}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex">
                    <div class="w-1/2">
                        ...
                        @foreach($act as $a)
                            <option value="{{ $a->id }}" {{ ($a->id == 5)? "selected":"" }}>{{ $a->name }}</option>
                        @endforeach
                        </select>
                    </div>
            ...
            <input type="datetime-local" class="h-10 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->has('start')? 'is-invalid' : '' }}" id="start" name="start" placeholder="Enter start time/date" value="{{old('start', Carbon\Carbon::now()->format('Y-m-d\TH:i')) }}"/>
            @if($errors->has('start'))
                <div class="error-fb">
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" class="h-10 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->has('end')? 'is-invalid' : '' }}" id="end" name="end" value="{{old('end', Carbon\Carbon::now()->addMinutes(30)->format('Y-m-d\TH:i')) }}"/>
                @if($errors->has('end'))
                <div class="error-fb">
            ...
            <div id="portfolio_competency_component" data-comps="{{json_encode($comps)}}" ></div>
